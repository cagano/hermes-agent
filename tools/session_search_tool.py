...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "TWO MODES:\n"
        "1. Recent sessions (no query): returns titles, previews, timestamps. Zero LLM cost. "
        "Use when user asks 'what did we work on recently'.\n"
        "2. Keyword search (with query): returns LLM summaries of matching sessions.\n\n"
        "Use proactively when user says 'we did this before', 'last time', 'remember when', "
        "or references something not in current context. Fast and cheap — search before asking.\n\n"
        "Search syntax: use OR for broad recall (elevenlabs OR baseten), "
        "quotes for phrases (\"docker networking\"), prefix (deploy*). "
        "FTS5 defaults to AND — use OR to avoid missing sessions."
    ),
    "parameters": {
        "type": "object",
        "properties": {
